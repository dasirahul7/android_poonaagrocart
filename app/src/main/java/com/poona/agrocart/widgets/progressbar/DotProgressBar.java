package com.poona.agrocart.widgets.progressbar;
...
public class DotProgressBar extends View {
    ...
    private void applyAttrs(Context context, AttributeSet attrs) {
        TypedArray a = context.getTheme().obtainStyledAttributes(
                attrs, R.styleable.DotProgressBar, 0, 0);

        try {
            mDotColor = a.getColor(R.styleable.DotProgressBar_color, mDotColor);
            mDotCount = a.getInteger(R.styleable.DotProgressBar_count, mDotCount);
            mDotCount = Math.min(Math.max(mDotCount, MIN_COUNT), MAX_COUNT);
            mTimeout = a.getInteger(R.styleable.DotProgressBar_timeout, mTimeout);
